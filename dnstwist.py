...
def _debug(msg):
    if "DEBUG" in os.environ:
        if isinstance(msg, Exception):
            print(
                "{}:{} {}".format(__file__, msg.__traceback__.tb_lineno, str(msg)),
                file=sys.stderr,
                flush=True,
            )
        else:
...
GEOLITE2_MMDB = os.environ.get(
    "GEOLITE2_MMDB", os.path.join(os.path.dirname(__file__), "GeoLite2-Country.mmdb")
)
try:
...
VALID_FQDN_REGEX = re.compile(
    r"(?=^.{4,253}$)(^((?!-)[a-z0-9-]{1,63}(?<!-)\.)+[a-z0-9-]{2,63}$)", re.IGNORECASE
)

USER_AGENT_STRING = "Mozilla/5.0 ({} {}-bit) dnstwist/{}".format(
    sys.platform, sys.maxsize.bit_length() + 1, __version__
)
...
class Whois:
    ...
    def _extract(self, response):
        ...
        response_reduced = "\r\n".join(
            [x.strip() for x in response.splitlines() if not x.startswith("%")]
        )
    ...
    def query(self, query, server=None):
        ...
        refer = re.search(
            r"refer:\s+(?P<server>[-.a-z0-9]+)", response, re.IGNORECASE | re.MULTILINE
        )
        if refer:
    ...
    def __lt__(self, other):
        if self["fuzzer"] == other["fuzzer"]:
            if len(self) > 2 and len(other) > 2:
                return (
                    self.get("dns_a", [""])[0] + self["domain"]
                    < other.get("dns_a", [""])[0] + other["domain"]
                )
            else:
                return self["domain"] < other["domain"]
...
class HeadlessBrowser:
    ...
    def __init__(self, useragent=None):
        ...
        if proxies:
            proxy_string = ";".join(
                ["{}={}".format(scheme, url) for scheme, url in proxies.items()]
            )
        ...
        self.driver.execute_cdp_cmd(
            ...
            {
                "userAgent": useragent
                or self.driver.execute_script("return navigator.userAgent").replace(
                    "Headless", ""
                )
            },
        )
...
class Fuzzer:
    ...
    def _homoglyph(self):
        md = lambda a, b: {
            k: set(a.get(k, [])) | set(b.get(k, []))
            for k in set(a.keys()) | set(b.keys())
        }
        glyphs = md(
            self.glyphs_ascii, self.glyphs_idn_by_tld.get(self.tld, self.glyphs_unicode)
        )
    ...
    def _hyphenation(self):
        return {
            self.domain[:i] + "-" + self.domain[i:] for i in range(1, len(self.domain))
        }

    def _insertion(self):
        result = set()
        for i in range(0, len(self.domain) - 1):
            prefix, orig_c, suffix = (
                self.domain[:i],
                self.domain[i],
                self.domain[i + 1 :],
            )
            for c in (c for keys in self.keyboards for c in keys.get(orig_c, [])):
                result.update(
                    {prefix + c + orig_c + suffix, prefix + orig_c + c + suffix}
                )
        return result
    ...
    def _repetition(self):
        return {
            self.domain[:i] + c + self.domain[i:] for i, c in enumerate(self.domain)
        }
    ...
    def _transposition(self):
        return {
            self.domain[:i] + self.domain[i + 1] + self.domain[i] + self.domain[i + 2 :]
    ...
    def _plural(self):
        for i in range(2, len(self.domain) - 2):
            yield self.domain[: i + 1] + (
                "es" if self.domain[i] in ("s", "x", "z") else "s"
            ) + self.domain[i + 1 :]
    ...
    def _addition(self):
        ...
        if "-" in self.domain:
            ...
            result = {
                "-".join(parts[:p]) + chr(i) + "-" + "-".join(parts[p:])
                ...
                self.domains.add(
                    Permutation(
                        ...
                        domain=".".join(
                            filter(
                                None,
                                [self.subdomain, self.domain, self.tld.split(".")[-1]],
                            )
                        ),
                    )
                )
            ...
            if "." not in self.tld:
                self.domains.add(
                    Permutation(
                        ...
                        domain=".".join(
                            filter(
                                None, [self.subdomain, self.domain + self.tld, self.tld]
                            )
                        ),
                    )
                )
            if self.tld != "com" and "." not in self.tld:
                self.domains.add(
                    Permutation(
                        ...
                        domain=".".join(
                            filter(
                                None,
                                [self.subdomain, self.domain + "-" + self.tld, "com"],
                            )
                        ),
                    )
                )
                self.domains.add(
                    Permutation(
                        ...
                        domain=".".join(
                            filter(
                                None, [self.subdomain, self.domain + self.tld, "com"]
                            )
                        ),
                    )
                )
            if self.subdomain:
                ...
                self.domains.add(
                    Permutation(
                        ...
                        domain=".".join(
                            [
                                self.subdomain.replace(".", "-") + "-" + self.domain,
                                self.tld,
                            ]
                        ),
                    )
    ...
    def permutations(
        self, registered=False, unregistered=False, dns_all=False, unicode=False
    ):
...
class Scanner(threading.Thread):
    ...
    def _banner_http(self, ip, vhost):
        response = self._send_recv_tcp(
            ip,
            80,
            "HEAD / HTTP/1.1\r\nHost: {}\r\nUser-Agent: {}\r\n\r\n".format(
                vhost, self.useragent
            ).encode(),
        )
        if not response:
            return ""
                            ...
                            if self.option_lsh == "ssdeep":
                                ...
                                if lsh_curr not in (None, "", "TNULL"):
                                    task["tlsh"] = int(
                                        100
                                        - (
                                            min(tlsh.diff(self.lsh_init, lsh_curr), 300)
                                            / 3
                                        )
                                    )
    ...
    def cli(self):
        ...
        for domain in domains:
            ...
            if "dns_mx" in domain:
                inf.append(
                    kv(
                        "SPYING-MX:" if domain.get("mx_spy") else "MX:",
                        ";".join(domain["dns_mx"]),
                    )
                )
            ...
            cli.append(
                "{}{[fuzzer]:<{}}{} {[domain]:<{}} {}".format(
                    FG_BLU, domain, wfuz, FG_RST, domain, wdom, " ".join(inf or ["-"])
                )
            )
        return "\n".join(cli)
...
def run(**kwargs):
    ...
    parser.add_argument(
        ...
        "--all",
        action="store_true",
        help="Print all DNS records instead of the first ones",
    )
    parser.add_argument(
        "-b",
        "--banners",
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="LIST",
        help="Generate more domains using keywords (separated with commas)",
    )
    parser.add_argument(
        "-f",
        "--format",
        type=str,
        default="cli",
        help="Output format: cli, csv, json, list (default: cli)",
    )
    parser.add_argument(
        "--fuzzers",
        type=str,
        metavar="LIST",
        help="Use only selected fuzzing algorithms (separated with commas)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--mxcheck",
        action="store_true",
        help="Check if MX host can be used to intercept emails",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--unregistered",
        action="store_true",
        help="Show only unregistered domain names",
    )
    parser.add_argument(
        "-p",
        "--phash",
        action="store_true",
        help="Render web pages and evaluate visual similarity",
    )
    parser.add_argument(
        "--phash-url",
        metavar="URL",
        help="Override URL to render the original web page from",
    )
    parser.add_argument(
        "--screenshots", metavar="DIR", help="Save web page screenshots into DIR"
    )
    ...
    parser.add_argument(
        "--ival", type=int, metavar="NUM", help="Delay between progress yields"
    )
    parser.add_argument(
        "-t",
        "--threads",
    ...
    parser.add_argument(
        ...
        "--whois",
        action="store_true",
        help="Lookup WHOIS database for creation date and registrar",
    )
    parser.add_argument(
        "--tld",
        type=str,
        metavar="FILE",
        help="Swap TLD for the original domain from FILE",
    )
    parser.add_argument(
        "--nameservers",
        type=str,
        metavar="LIST",
        help="DNS or DoH servers to query (separated with commas)",
    )
    parser.add_argument(
        "--useragent",
        type=str,
        metavar="STRING",
        default=USER_AGENT_STRING,
        help="Set User-Agent STRING (default: %s)" % USER_AGENT_STRING,
    )
    parser.add_argument(
        "--version",
        action="version",
        version="dnstwist {}".format(__version__),
    ...
    def p_cli(text):
        if args.format == "cli" and sys.stdout.isatty():
    ...
    if args.fuzzers:
        ...
        if args.dictionary and "dictionary" not in fuzzers:
            parser.error(
                "argument --dictionary cannot be used with selected fuzzing algorithms (consider enabling fuzzer: dictionary)"
            )
        if args.tld and "tld-swap" not in fuzzers:
            parser.error(
                "argument --tld cannot be used with selected fuzzing algorithms (consider enabling fuzzer: tld-swap)"
            )
        ...
        with Fuzzer("example.domain", ["foo"], ["bar"]) as fuzz:
            ...
            if not set(fuzzers).issubset(all_fuzzers):
                parser.error(
                    "argument --fuzzers takes a comma-separated list with at least one of the following: {}".format(
                        " ".join(all_fuzzers)
                    )
                )
            del all_fuzzers
    ...
    if args.dictionary:
        ...
        try:
            with open(args.dictionary, encoding="utf-8") as f:
                dictionary = [
                    x for x in set(f.read().lower().splitlines()) if re_subd.match(x)
                ]
        except UnicodeDecodeError:
            parser.error("UTF-8 decode error when reading: {}".format(args.dictionary))
        except OSError as err:
            parser.error(
                "unable to open {} ({})".format(args.dictionary, err.strerror.lower())
            )
    if args.keywords:
        dictionary.extend(
            [x.strip().lower() for x in set(args.keywords.split(",")) if x.strip()]
        )
    tld = []
    if args.tld:
        ...
        try:
            ...
            parser.error(
                "unable to open {} ({})".format(args.tld, err.strerror.lower())
            )
    ...
    if args.output:
        ...
        try:
            ...
            parser.error(
                "unable to open {} ({})".format(args.output, err.strerror.lower())
            )
    ...
    if args.geoip:
        if not MODULE_GEOIP:
            parser.error(
                "missing geoip2 library or database file (check $GEOLITE2_MMDB environment variable)"
            )
    ...
    if not domains:
        parser.error(
            "selected fuzzing algorithms do not generate any permutations for provided input domain"
        )
...
""" % __version__ + FG_RST + ST_RST)
    ...
    while True:
        ...
        p_cli(
            ST_CLR
            + "\rpermutations: {:.2%} of {} | found: {} | eta: {:d}m {:02d}s | speed: {:d} qps".format(
                comp / dlen, dlen, found, eta // 60, eta % 60, rate
            )
        )
        if jobs.empty():
            break
    ...
    domains = fuzz.permutations(
        registered=args.registered, unregistered=args.unregistered, dns_all=args.all
    )
        ...
        for i, domain in enumerate([x for x in domains if x.is_registered()]):
            ...
            try:
                ...
                if wreply.get("creation_date"):
                    domain["whois_created"] = wreply.get("creation_date").strftime(
                        "%Y-%m-%d"
                    )
